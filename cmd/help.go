...
import (
	...
	u "github.com/saltkid/tbg/utils"
)
...
func HelpExecute(c *Cmd) error {
	if len(c.SubCmds) == 0 && len(c.Flags) == 0 {
		fmt.Printf("%-37s%s\n", *u.Decorate("tbg").Bold().Underline(),
			*u.Decorate("Terminal Background Gallery").Italic())
		fmt.Printf("%-30s%s\n", "", fmt.Sprintf("%s (%s) allows the user to have and manage multiple background", *u.Decorate("tbg").Bold(), *u.Decorate("teabag").Italic()))
		fmt.Printf("%-30s%s\n", "", fmt.Sprintf("images, that rotate at a set amount of time, for Windows Terminal."))
		fmt.Printf("%-37s%s\n", *u.Decorate("Version").Bold().Underline(),
			*u.Decorate(TbgVersion).Italic())
		fmt.Printf("\n%-37s%s\n", *u.Decorate("Usage").Bold().Underline(),
			*u.Decorate("tbg run").Italic())
		fmt.Printf("\n%s:\n", *u.Decorate("Commands").Bold().Underline())
		RunHelp(false)
		AddHelp(false)
		RemoveHelp(false)
		EditHelp(false)
		...
		OpacityHelp(false)
		RandomHelp(false)
		fmt.Printf("\n%s\n", *u.Decorate("Not all flags are applicable to all commands. See help <command> for more info").Italic())
		return nil
	}
...
func AddHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  add").Bold(),
...
func RemoveHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  remove").Bold(),
	...
	if verbose {
		...
		fmt.Println("      --------------------------                    --------------------------------------------")
		fmt.Println("      | image_col_paths:                            | image_col_paths:")
		fmt.Println("      |   - /path/to/images/dir | center none 0.1   |   - /path/to/images/dir | right none 0.1")
		fmt.Println("      |                                             |")
		fmt.Println("      | default_alignment: right                    | default_alignment: right")
		fmt.Println("      | default_stretch: fill                       | default_stretch: fill")
		fmt.Println("      | default_alignment: 0.2                      | default_alignment: 0.2")
		...
		fmt.Println("      will be \"removed\" by inheriting the value of \"default_alignment\". The other flags are untouched")
		fmt.Println("\n  4. tbg remove path/to/images/dir --stretch --opacity")
		fmt.Println("      before:                          after:")
		fmt.Println("      --------------------------                    --------------------------------------------")
		fmt.Println("      | image_col_paths:                            | image_col_paths:")
		fmt.Println("      |   - /path/to/images/dir | right none 0.1    |   - /path/to/images/dir")
		fmt.Println("      |                                             |")
		fmt.Println("      | default_alignment: right                    | default_alignment: right")
		fmt.Println("      | default_stretch: fill                       | default_stretch: fill")
		fmt.Println("      | default_alignment: 0.2                      | default_alignment: 0.2")
		...
		fmt.Println("      | other fields...                             | other fields...")
		fmt.Println("      --------------------------                    ---------------------------------------------")
		fmt.Println("      All flags were removed from the path because alignment was already defaulted and you specified")
		fmt.Println("      --stretch and --opacity. This means stretch and opacity will be defaulted too. So instead of keeping")
		fmt.Println("      all 3 as default values \"right fill 0.2\", when doing \"tbg remove\", tbg will just remove them")
		fmt.Println("      as if you specified all 3 flags in the first place. See example 2")
	}
}

func EditHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  edit").Bold(),
...
func ConfigHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  config").Bold(),
...
func ProfileHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  -p, --profile").Bold(),
...
func IntervalHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  -i, --interval").Bold(),
...
func AlignmentHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  -a, --alignment").Bold(),
...
func StretchHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  -s, --stretch").Bold(),
...
func OpacityHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  -o, --opacity").Bold(),
...
func RandomHelp(verbose bool) {
	fmt.Printf("%-33s%s", *u.Decorate("  -r, --random").Bold(),
